#!/bin/bash
# TEST 2: 5000 pasazerow, 4999 ma niebezpieczny przedmiot, 1 (id=0) nie ma
# - stanowisko kontroli wykrywa i konfiskuje - pasazer nadal wsiada
# - konfiguracja identyczna z test1 (3 promy x 500, brak VIP, bagaz 1kg)
# Oczekiwany wynik: 5000 BOARDED, 4999 "DANGEROUS ITEM found", pasazer id=0 bezpieczny

set -e

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
PROJECT_DIR="$(dirname "$SCRIPT_DIR")"
TMP_DIR="/tmp/prom_test2_$$"
LOG_FILE="$PROJECT_DIR/simulation_test.log"

echo "=== TEST 2: 5000 pasazerow, 4999 z niebezpiecznym przedmiotem ==="
...
# === security_station.cpp: deterministyczne wyniki inspekcji ===
# Pasazer id=0 -> brak niebezpiecznego przedmiotu
# Pasazerowie id=1..4999 -> zawsze niebezpieczny przedmiot
sed -i 's|insp.dangerous_item = (rand() % 100 < DANGEROUS_ITEM_CHANCE) ? 1 : 0;|insp.dangerous_item = (admitted.passenger_id == 0) ? 0 : 1;|' "$TMP_DIR/security_station.cpp"
...
> "$LOG_FILE"
echo "Symulacja 5000 pasazerow, pasazer id=0 bezpieczny, reszta z niebezpiecznym (log: $LOG_FILE)"
cd "$TMP_DIR"
...
DANGEROUS_FOUND=$(grep -c "DANGEROUS ITEM found on passenger" "$LOG_FILE" || true)
echo "DANGEROUS ITEM found lacznie:   $DANGEROUS_FOUND / 4999"

PASSENGER0_BOARDED=$(grep -c "id=0 BOARDED" "$LOG_FILE" || true)
echo "Pasazer id=0 BOARDED:           $PASSENGER0_BOARDED (oczekiwane: 1)"

PASSENGER0_DANGEROUS=$(grep -c "DANGEROUS ITEM found on passenger 0" "$LOG_FILE" || true)
echo "Pasazer id=0 DANGEROUS ITEM:    $PASSENGER0_DANGEROUS (oczekiwane: 0)"

PASS=true

if [ "$BOARDED" -ne 5000 ]; then
    echo "FAIL: Oczekiwano 5000 BOARDED, uzyskano $BOARDED"
    PASS=false
fi

if [ "$DANGEROUS_FOUND" -ne 4999 ]; then
    echo "FAIL: Oczekiwano 4999 DANGEROUS ITEM found, uzyskano $DANGEROUS_FOUND"
    PASS=false
fi

if [ "$PASSENGER0_BOARDED" -lt 1 ]; then
    echo "FAIL: Pasazer id=0 powinien wejsc na prom"
    PASS=false
fi

if [ "$PASSENGER0_DANGEROUS" -ne 0 ]; then
    echo "FAIL: Pasazer id=0 nie powinien miec wykrytego niebezpiecznego przedmiotu"
    PASS=false
fi

if [ "$PASS" = true ]; then
    echo ""
    echo "WYNIK: PASS - Wszyscy 5000 wsiada, 4999 niebezpiecznych skonfiskowanych, id=0 bezpieczny"
else
    echo ""
    echo "WYNIK: FAIL"
fi
